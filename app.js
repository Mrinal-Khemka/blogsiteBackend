...
//console.log(homeStartingContent);
...
app.get('/posts/:route', function (req, res) {
   let routeTitle =_.lowerCase(req.params.route);
  posts.forEach(function(post)
  {
     let postTitle=_.lowerCase(post.title);
    
    if(routeTitle===postTitle)
    {
      res.render("post",{postTitle:post.title,postContent:post.postContent});
    }
  });
...
{
  Blog.findOne({title:"Home"},function(err,foundBlog)
  {
    res.render("home",{homeContent:foundBlog.content,posts:posts});
  });
...
app.post("/compose",function(req,res)
{
  ...
  const post={
   ...
   Post.findOne({posts:post},function(err,foundPost)
   {
       if(!err)
       {
         if(!foundPost)
